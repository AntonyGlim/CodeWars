    ...
    public static List<String>  movingShift(String s, int shift) {
        ...
        for (int i = 0; i < chars.length; i++) {
            if ((chars[i] >= 65 && chars[i] <= 90) || (chars[i] >= 97 && chars[i] <= 122)){
                chars[i] = (char) (chars[i] + shift % 26 - 26 * (shift / 26));
            }
            shift++;
        }
...
//    }
    ...
    public static void characterNumbers(){
        System.out.println("a: " + (int) new Character('a'));
        System.out.println("z: " + (int) new Character('z'));
        System.out.println("A: " + (int) new Character('A'));
        System.out.println("Z: " + (int) new Character('Z'));
        System.out.println("{: " + (int) new Character('{'));
    }

    public static Character formula(Character ch, int shift){
        return (char) (96 + (ch - 96 + shift) % 26);
    }
}
